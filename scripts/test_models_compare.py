...
def _eval_lgbm_for_venue(df: pd.DataFrame, venue: str) -> Dict[str, float]:
    model_path = MODEL_DIR / f"trifecta120_lgbm_{venue}.joblib"
    meta_path = MODEL_DIR / f"trifecta120_lgbm_{venue}_meta.json"
    labels_path = MODEL_DIR / f"trifecta120_lgbm_{venue}_labels.json"
...
def main() -> None:
    ...
    for venue in TARGET_VENUES:
        try:
            ...
            summary["LightGBM"][venue] = lgbm_result

        except Exception as e:
            summary["RandomForest"][venue] = {"error": str(e)}
            summary["LightGBM"][venue] = {"error": str(e)}
