...
def fetch_racelist_common(
    *,
    ...
    cache_key = f"{venue_key}_racelist_{date}_{race_no}"
    cached = _get_cache(_RACE_CACHE, cache_key, _CACHE_SECONDS_RACE)
    if cached is not None:
        return cached

    url = f"https://www.boatrace.jp/owpc/pc/race/racelist?hd={date}&jcd={int(jcd):02d}&rno={int(race_no)}"
...
def fetch_all_entries_common(
    *,
    ...
    cache_key = f"{venue_key}_all_entries_{date}"
